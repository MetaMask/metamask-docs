...
- A new `generateSalt` utility generates a random 32-byte hex salt for creating delegations, helping
...
- A new ERC-7715 `token-approval-revocation` permission type and `CaveatBuilder` for
  `ApprovalRevocationEnforcer` are now available for revoking token approvals.
- A new helper decodes revert reasons from delegated execution errors while preserving the original
  error output.

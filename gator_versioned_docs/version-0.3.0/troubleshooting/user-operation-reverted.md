...
2. **Simulate directly** — Call the target contract directly using `eth_call` from the smart
   account address. This bypasses the `EntryPoint` and surfaces more detailed errors.
3. **Use Tenderly** — Simulate the user operation in [Tenderly](https://tenderly.co) to get a full
   execution trace. The trace view shows the exact line where the inner call reverts.
4. **Check the basics** — Verify the target has deployed code, the smart account has enough

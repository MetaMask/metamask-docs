...
## Integrating Embedded Wallets in Android

Once, you have set up your Embedded Wallets dashboard, and created a new project, it's time to integrate the 
Web3Auth SDK in your Android application. For the implementation, we'll use the 
...
1. In your module-level `build.gradle` or `settings.gradle` file, add the JitPack repository.
...
Once we have the created `Web3AuthHelper`, the next step is to initialize the `Web3Auth` instance in the 
...
[Learn more about Embedded Wallets  session management](/embedded-wallets/features/session-management).
...
Learn more on [integrating different blockchains with Embedded Wallets](/embedded-wallets/connect-blockchain/).
...
### Ethereum use case implementation

Once we have generated `Token` wrapper, we'll create `EthereumUseCaseImpl` and implement the methods. To 
create the `Web3j` instance, you'll require the RPC target URL. If you are using public RPCs, you can face 
some network congestion. It's ideal to use [paid RPCs](/services) for production.
...
help us manage the state and functionality of the wallet. It will help us manage the state of the currently 
...
The next step is to set up chain interactions for signing messages, signing transactions, reading from contracts, 
and writing on contracts. 

1. For signing messages and transaction, we'll create a new `TransactionScreen` widget and utilize `signMessage` and 
...
2. Next, we create a `SmartContractsScreen` for fetching ERC-20 token balances, and revoking approval. 

...
In this guide, we'll talk about how we can use MetaMask Embedded Wallets SDK (formerly Web3Auth Plug and Play) 
to build your Ethereum Web3 wallet in Android. The wallet will only support the Ethereum ecosystem, but 
functionality can be extended with any blockchain ecosystem.

You will create a simple app with the functionality to log in, display user details, and perform blockchain 
interactions. The signing of the blockchain transactions is done through the Embedded Wallets SDK. You 
can check out the infrastructure docs, 
[Embedded Wallets management infrastructure](/embedded-wallets/infrastructure/) for a high-level overview 
of the Embedded Wallets architecture and implementation. For those who want to skip straight to the code, you can 
...
## How to set up Embedded Wallets dashboard

If you haven't already, sign up on the [Embedded Wallets platform](https://dashboard.web3auth.io/). It's free 
and gives you access to the Embedded Wallets'base plan. After the basic setup, explore other features and 
functionalities offered by Embedded Wallets. 
...
Once, you have set up your Embedded Wallets dashboard, and created a new project, it's time to integrate Web3Auth 
in your Android application. For the implementation, we'll use the 
...
1. To install the web3auth-android-sdk SDK,in your module-level `build.gradle` or `settings.gradle` file, 
...
2. In your app-level `build.gradle` dependencies section, add the `web3auth-android-sdk`.

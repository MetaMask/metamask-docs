...
In the near future, server wallets will even be a requirement for non-web2-native developers.
This necessitates thinking about an ideal architecture for server wallets.

This page outlines an opinionated, production‑oriented architecture you can adopt today and hints at where MetaMask is going next with a one-click ERC‑8004 server wallet experience.

## What is a server wallet?

A server wallet is a backend signer that holds and uses private keys on behalf of an app or AI agent, while clients authenticate requests and receive signatures to submit onchain.
...
You can refer to the following high-level architecture when designing a server wallet:

- The client holds an *agent key* used to authenticate who is asking to sign; separate from the onchain account key that actually controls funds.​
- The backend exposes minimal APIs that forward requests into a trusted execution environment (TEE) — for example, [AWS Nitro Enclaves](https://aws.amazon.com/ec2/nitro/nitro-enclaves/).
...
The following is an example request flow:
...
The following are descriptions of security operations related to managing and using keys.
...
2. The TEE validates the request, enforces policy, and signs the request.
3. The TEE returns the encrypted blog stored in the database, which can be decrypted on the client side with the agent key.​
...
You can refer to following TEE policies when implementing security operations:

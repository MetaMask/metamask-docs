...
and gives you access to the Embedded Wallets' base plan. 
...
For interacting with Ethereum chains, we'll use the [web3dart](https://pub.dev/packages/web3dart) package. While for Solana, we'll use the [Solana](https://pub.dev/packages/solana) package. 
...
3.2 Set up our Blockchain provider. First, we'll create a new class, `ChainProvider`, which will used as a base class for `EthereumProvider` and `SolanaProvider`. If you wish to support any additional ecosystem, you can extend the `ChainProvider` and implement the methods.

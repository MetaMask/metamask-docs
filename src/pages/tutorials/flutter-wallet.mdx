...
description: 'Empower your Flutter app with a chain-agnostic web3 wallet using the Embedded Wallets SDK | Web3Auth PnP SDK.'
...
You'll create a demo app which supports user sign-in, displays user details, and performs blockchain interactions. Blockchain transaction signing is done through the Embedded Wallets SDK.
...
Sign up on the [Embedded Wallets dashboard](https://developer.metamask.io/). It's free
...
If the user isn't authenticated, call the `login` method. Provide two sign-in options for the wallet: Google and passwordless email sign-in. Embedded Wallets supports two authentication flows: single page authentication, and regular web application. In this guide, you use the single-page authentication flow.
We'll create a helper function, `_login` inside `LoginScreen`. The sign-in flow uses `LoginParams` as input. After a successful sign-in, navigate the user to `HomeScreen`.

:::tip

Learn more about [Embedded Wallets' `LoginParams`](/embedded-wallets/sdk/flutter/usage/connect-to#parameters).
...
class _LoginScreenState extends State<LoginScreen> with WidgetsBindingObserver {
  ...
  Future<void> _login(BuildContext context) async {
    try {
      ...
      // authConnection. For instance, you'll need to pass user's email address as
      // login_hint when the auth connection is email_passwordless.
...
If you are using public RPCs, you could face some network congestion. Ideally, use [paid RPCs](https://docs.infura.io/) for production.
...
class EthereumProvider extends ChainProvider {
  ...
  // secp256k1 curve. You can use the Web3AuthFlutter.getPrivateKey
...
### 4.1 Define the supported chains
...
### 4.2 Create a model to represent the Dart object
...
5.3.3 In `HomeScreen` we'll also give users an option to sign out from the wallet in navigation drawer. To do so, we'll utilize the `Web3AuthFlutter.logout`. Upon success, we'll navigate users back to `LoginScreen`. Checkout `SideDrawer` widget for navigation drawer implementation.

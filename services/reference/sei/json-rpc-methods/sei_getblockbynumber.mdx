...
Returns block information by number, including synthetic transactions from Cosmos SDK modules. <CreditCost network="evm" method="sei_getBlockByNumber" />

:::info
This method is identical to the [`eth_getBlockByNumber`](./eth_getblockbynumber.mdx) method, but it
includes synthetic transactions that are marked with a `synthetic` field set to `true`.
:::

## Parameters

- `block parameter`: (string) [_Required_] A hexadecimal block number, or one of the string tags `earliest`, `latest`, `pending`

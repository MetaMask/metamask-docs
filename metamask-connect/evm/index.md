...
MetaMask Connect EVM provides an EIP-1193 compatible provider that works with viem (via `custom()` transport), ethers.js (via `BrowserProvider`), web3.js (via `Web3` constructor), and wagmi (via the MetaMask connector). It also supports wallet connector libraries like RainbowKit, ConnectKit, Dynamic, and Web3Auth.

...
All changes made within the Branding tab can be previewed in real-time. However, to reflect updates in production, developers must click the Save & Publish button after making changes.
:::
...
Enable this option to automatically detect and display any compatible wallets already installed in the user's browser or device (such as, MetaMask or Phantom). This improves onboarding by surfacing options users are already familiar with.

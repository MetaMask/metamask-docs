---
title: Nodes and DKG
sidebar_label: Nodes & DKG
description: 'DKG Nodes in Wallet Management | Embedded Wallets | Torus Network'
---
...
The Torus Node Network runs a [Distributed Key Generation](./glossary.mdx#distributed-key-generation) (DKG) protocol to assign, store and return secrets/keys to users. The nodes manage a key share, retrieved via conventional authentication flows (such as OAuth, password, or other supported authentication).
...
<img className="docImg" src="/img/tutorials/graph-6-final.png" alt="Architecture" />

A smart contract is used for node discovery. Nodes are selected, [operate for a fixed period](./glossary.mdx#epochs), and generate a set of keys via DKG.
...
<!-- The phrase "reconstructs the user's key in the frontend" suggests that this section is talking about SSS (reconstruction is expected), not the TSS/MPC flow. Let's make clear which flow. In an MPC/TSS design, the private key should never be reconstructed client-side. IF this is TSS flow perhaps should say something like: "The client combines the required shares as part of a threshold signing protocol, ensuring that the full private key is never reconstructed or exposed"? Also, can we clarify whether "frontend" refers to the dapp client or a Torus-controlled client environment? -->
...
<img
  className="docImg"
  src="/img/tutorials/login-key-assignments.png"
  alt="Standard login with OAuth2 (Google, Facebook, Apple, Reddit, Discord, Twitch)"
/>
...
<!-- Per comment on text at line 30, this wording could imply that encrypted key shares are returned to the frontend in a way that allows full key reconstruction. Can we clarify whether or not the private key is reconstructed SSS (reconstruction is expected) vs. the TSS/MPC flow and, if it is, how it is protected from exposure, and again confirm what "frontend" refers to in this context (the dapp/sdk runtime)? -->

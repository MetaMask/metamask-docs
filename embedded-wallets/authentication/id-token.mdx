...
To retrieve the ID token on the client-side, use the `getAuthTokenInfo()` method. This is typically called after the user logs in.
...
import { useAuthTokenInfo, useWeb3Auth, useWeb3AuthConnect } from '@web3auth/modal/react'
...
const validateIdToken = async () => {
  ...
  if (connectorName === 'auth') {
    ...
    const pubKey = await connection?.ethereumProvider?.request({
      method: 'public_key',
    })
    ...
    //focus-start
    // External wallet: send address (from Wagmi, not eth_accounts on the provider)

...
Note that polyfilling is not supported with "Expo Go" app. It's compatible only with Custom Dev Client and Expo Application Services (EAS) builds. Please [prebuild your expo app](https://docs.expo.dev/workflow/continuous-native-generation/#usage) 
...
2. Import the dependencies to `index.js` of your project.

:::info

For Expo apps, you need to create an entry point, that is, the index.js file. This can be done by following 
the [naming guide](https://docs.expo.dev/versions/latest/sdk/expo/#what-if-i-want-to-name-my-main-app-file-something-other-than-appjs-or-app_layouttsx)

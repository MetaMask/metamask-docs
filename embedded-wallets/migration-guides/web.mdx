...
`@web3auth/modal/vue/wagmi` and never reads `useWeb3Auth().provider`, you may not need EVM code
changes beyond the [Wagmi v3 upgrade](#install-v11).
...
| EVM in React/Vue          | `provider.request(...)`                                 | Wagmi hooks (`useAccount`, `useBalance`, etc.)                                       |

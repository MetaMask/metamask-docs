...
```bash npm2yarn
npm install --save @web3auth/modal @taquito/signer @taquito/taquito @taquito/utils @tezos-core-tools/crypto-utils
```
...
Using the function, `web3auth.connection?.ethereumProvider?.request({ method: 'private_key' })` from Web3Auth provider, the application can have access to the user's private key. However, we cannot use this key with Tezos EC Curve specific signing functions, hence, we first derive the Tezos Key using the `getTezosKeyPair()` function.

Under the hood, it uses the `tezosCrypto.utils.seedToKeyPair()` function, where we need to pass the `hex2buf(privateKey)`, that is, the hexadecimal to byteArray converted private key. We can use the returned private key pair from this and use on Tezos transactions.

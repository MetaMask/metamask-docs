...
```bash npm2yarn
npm install --save @web3auth/modal @polkadot/keyring @polkadot/util-crypto
```
...
Using the function, `web3auth.connection?.ethereumProvider?.request({ method: 'private_key' })` from Web3Auth provider, the application can have access to the user's private key. However, we cannot use this key with Polkadot-specific signing functions, hence, we first derive the Polkadot Keyring using the `Keyring` class.

...
```bash npm2yarn
npm install --save @web3auth/modal @mysten/sui.js
```
...
Using the function, `web3auth.connection?.ethereumProvider?.request({ method: 'private_key' })` from Web3Auth provider, the application can have access to the user's private key. However, Web3Auth does not provide direct access to Sui specific signing functions, hence, we create a new `Ed25519Keypair` to give SDK the ability to sign transactions with this key pair.

---
title: Integrate Embedded Wallets with the TON Blockchain
sidebar_label: TON
image: 'guides/img/embedded-wallets/banners/ton.png'
keywords: [ton, embedded-wallets, authentication, blockchain]
description: 'Integrate Embedded Wallets with the TON Blockchain | Embedded Wallets'
---

import InitializeWeb3Auth from '../_initialize-web3auth-other-pnp.mdx'

While using the Embedded Wallets Web SDK (formerly Web3Auth) for a non-EVM chain like [TON](https://ton.org/), you can obtain the user's private key from the provider. Using this private key, you can use the corresponding libraries of the blockchain to make blockchain calls like getting the user's `account`, `balance`, `send transaction`, etc. We have highlighted a few methods here to get you started quickly.
...
```bash npm2yarn
npm install --save @web3auth/modal tonweb @orbs-network/ton-access
```

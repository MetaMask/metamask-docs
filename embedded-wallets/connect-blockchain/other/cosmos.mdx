...
description: 'Integrate Web3Auth with the Cosmos Blockchain | Embedded Wallets'
---
...
While using the Web3Auth Web SDK for a non-EVM chain like [Cosmos](https://cosmos.network/) you get a standard provider from which you can get the private key of the user. Using this private key, you can use the corresponding libraries of the blockchain to make blockchain calls like getting the user's `account`, `chain ID`, fetch `balance`, `send transaction`, etc. We have highlighted a few methods here to get you started quickly on that.

:::note

This reference is for the `Web`, however, you can use Cosmos on other Mobile and Gaming Platforms as well. Please follow our reference for [EVM Blockchains](/connect-blockchain/evm/), and similarly use Cosmos libraries that support the platforms to use the private key and make blockchain calls accordingly.

...
```bash npm2yarn
npm install --save @web3auth/modal web3 bn.js elliptic @starkware-industries/starkex-js @starkware-industries/starkware-crypto-utils
```
...
Using the function, `web3auth.connection?.ethereumProvider?.request({ method: 'private_key' })` from Web3Auth provider, the application can have access to the user's private key. However, we cannot use this key with StarkEx EC Curve specific signing functions, hence, we first derive the Stark Key using the `getStarkKey()` function.

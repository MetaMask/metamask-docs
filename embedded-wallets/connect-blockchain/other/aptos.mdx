---
title: Integrate Embedded Wallets with the Aptos Blockchain
sidebar_label: Aptos
image: 'img/embedded-wallets/banners/aptos.png'

keywords: [aptos, embedded-wallets, authentication, blockchain]
description: 'Integrate Embedded Wallets with the Aptos Blockchain | Embedded Wallets'
---
...
Using the function, `web3auth.connection?.ethereumProvider?.request({ method: 'private_key' })` from Web3Auth provider, the application can have access to the user's private key. However, we cannot use this key with Aptos EC Curve specific signing functions, hence, we first derive the Aptos Account using the `getAptosAccount()` function.

Under the hood, it uses the helper functions from the `aptos` library to derive the key pair.

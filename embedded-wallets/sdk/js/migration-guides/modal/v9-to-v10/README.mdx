...
**Wallet Services:** If you used the `@web3auth/wallet-services-plugin` in v9, see the 🛠️ **[Wallet Services Migration Guide](./wallet-services)** for migrating to the built-in integration.

## Additional Migration Guides

For specific functionality or frameworks, refer to these dedicated migration guides:

**📋 Whitelabeling and UI Customization:** If you had whitelabeling configurations (`uiConfig`, `modalConfig`, or `AuthAdapter` whitelabel settings) in v9, see the **[Whitelabeling Migration Guide](./whitelabeling)** for detailed steps.

**🔧 Smart Account Functionality:** If you used Smart Accounts in v9 with `@web3auth/account-abstraction-provider`, see the **[Smart Account Migration Guide](./smart-accounts)** for the new dashboard-centric approach.

**🔐 Custom Authentication:** If you used custom verifiers or aggregate verifiers in v9, see the **[Custom Authentication Migration Guide](./custom-authentication)** for migrating to the new Connections system.

**⚛️ React & Vue Applications:** If you're using React or Vue with Web3Auth, see the **[React & Vue Migration Guide](./react-vue)** for framework-specific migration steps including hooks, composables, and provider configurations.

...
[`EIP1193`](https://eips.ethereum.org/EIPS/eip-1193) compatible provider as
**`web3auth.connection?.ethereumProvider`** after successful authentication.
...
// provider is const provider = new ethers.providers.Web3Provider(connection?.ethereumProvider); from above.
...
const provider = new ethers.providers.Web3Provider(connection?.ethereumProvider)

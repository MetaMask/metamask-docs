...
connectTo<T extends WALLET_CONNECTOR_TYPE>(connector: T, params?: LoginParamMap[T]): Promise<void>
```

#### Return values

In Web SDK v11, `connect()` and `connectTo()` do not return the provider. After a successful
connection, read the EIP-1193 provider from `web3auth.connection?.ethereumProvider`.

...
- **`Connection | null`**: On successful sign-in, returns a `Connection` object with
  `ethereumProvider`, `solanaWallet`, and `connectorName`. On unsuccessful sign-in, returns `null`.
...
[JavaScript Ethereum integration](/embedded-wallets/sdk/js/ethereum-integration) guide.

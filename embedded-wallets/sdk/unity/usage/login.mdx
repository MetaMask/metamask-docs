...
| `leeway?`                    | The value used to account for clock skew in JWT expirations. The value is in the seconds, and ideally should be no more than 60 seconds, with a maxium of 120 seconds. String value.                                             |
...
| `isVerifierIdCaseSensitive?` | Bolean to confirm whether the verifier ID field is case sensitive or not.                                                                                                                                                        |
...
| `max_age?`                   | Maxium time allowed without reauthentication. If the last time the user authenticated is greater than this value, then the user must reauthenticate. String value.                                                               |
...
| `scope?`                     | The default scope to be used on authentication requests. The defaultScope defined in the Auth0Client is included along with this scope. String value.                                                                            |

...
description: "PnP iOS SDK - v8.1 to v8.2 | Documentation - Web3Auth"
...
`loginParams` from `launchWalletServices`.

## Changes in Detail

### `launchWalletServices` method updates

From v8.2, `loginParams` is removed from the `launchWalletServices` method. Developers now only need
to pass the `chainConfig` as the required argument.

  ...
  W3AInitParams(
    ...
    loginConfig: [
      Web3AuthProvider.JWT.rawValue: .init(
        verifier: "YOUR_VERIFIER_NAME", // Get it from Embedded Wallets/Web3Auth dashboard
...
To use the SMS Passwordless login, send the phone number as the `login_hint` parameter of the `ExtraLoginOptions`. Please make sure the phone number is in the format of +\{country_code}-\{phone_number}, that is, (+91-09xx901xx1).

...
The `loginConfig` parameter is a key value map. The key should be one of the `TypeOfLogin` in its string form, and the value should be a `LoginConfigItem` object.

To use custom authentication, first you'll need to configure your own verifier in the Web3Auth Dashboard in "Custom Authentication" section. [Learn how to create a custom verifier](/authentication).

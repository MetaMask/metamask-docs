...
Similarly to [providing transaction insights](../learn/tutorials/transaction-insights.md), Snaps allows you to provide signature insights to your users.
...
The `content` object must be an instance of [Custom UI](../features/custom-ui.md). The `severity`, if provided, must be an instance of the `SeverityLevel` enum exported by the `@metamask/snaps-sdk` package. Currently the only severity level available is `SeverityLevel.Critical`.

...
- **Private keys** - Avoid retrieving the user's private key from the Snap unless
  ...
  If you only need the user's public key, use [`snap_getBip32PublicKey`](../../reference/snaps-api/snap_getbip32publickey)
...
- `snap_confirm`, which is deprecated in favor of [`snap_dialog`](../../reference/snaps-api/snap_dialog).
...
- **Unsafe cryptographic libraries** - Avoid using unsafe cryptographic libraries.
  ...
  We recommend using [`snap_getEntropy`](../../reference/snaps-api/snap_getentropy) for entropy, the
